...
@Component({
    ...
    <ng-container *ngIf="!cargando && !errorCarga">
      <ng-container *ngIf="filtered | async as lista">
        ...
        <div *ngIf="lista.length === 0" class="estado">
          ...
          <button mat-stroked-button *ngIf="q.value" (click)="q.setValue('')">Quitar el filtro</button>
        </div>
      </ng-container>
    </ng-container>
  </div>
  ...
  styles: [`
    ...
    .estado mat-icon{font-size:40px;width:40px;height:40px;opacity:.6}
    .estado p{margin:0}
    .estado.error{color:var(--mat-sys-on-error-container)}
    ...
    .esqueleto .titulo{height:20px;width:70%}
    .esqueleto .corta{width:40%}
    @keyframes latido{0%,100%{opacity:.45}50%{opacity:.9}}
    @media (prefers-reduced-motion: reduce){
      .esqueleto .linea{animation:none}
    }
